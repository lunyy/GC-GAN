...
from load_data import load_data_list, load_real_data, load_graph_data, load_fake_data
...
def generate_pseudo_data(fold,timestamp,gen_no):
    ...
    [train_x, train_edge_index, train_label] = load_graph_data(fold=fold, data_filenames=real_filenames, timestamp=args.timestamp, pseudo_epoch=gen_no-1)
    ...
    while flag:
        ...
        d_prob = (np.amax(D_fake_class.cpu().detach().tolist(), axis=1))
...
def train_classifier(fold,train_x, train_edge_index, train_label,gen_no):
    ...
    # save log
    if not (os.path.isdir('GAN/Results_da/{}/model{}'.format(args.edge_type,args.timestamp))):
        os.makedirs(os.path.join('GAN/Results_da/{}/model{}'.format(args.edge_type,args.timestamp)))
    path_save_info = 'GAN/Results_da/{}/model{}'.format(args.edge_type,args.timestamp) + os.path.sep + "train_info{}_{}.csv".format(args.timestamp, fold)
...
if __name__ == "__main__":
    ...
    for i in range(1, args.gen_no + 1):
        for f1 in range(1, 6):
            # save log
            path_save_info = 'GAN/Results_da/model{}'.format(args.edge_type,args.timestamp) + os.path.sep + "train_info{}_{}.csv".format(args.timestamp, f1)
            with open(path_save_info.replace(".csv", "_test_{}.csv".format(i)), "w") as f:
        ...
        for f2 in range(1, 6):
            result_csv = pd.read_csv(
                'GAN/Results_da/{model{}/train_info{}_{}_test_{}.csv'.format(args.timestamp, args.timestamp, f2, i))
            ...
            with open('GAN/Results_da/model{}/cross_val_result{}_{}.csv'.format(args.timestamp, args.timestamp, i), 'a') as f:
                f.write('{},{},{},{},{}\n'.format(f2, round(test_result[1], 3) * 100, round(test_result[2], 3) * 100,
                                                  round(test_result[3], 3) * 100, round(test_result[4], 3) * 100))
            total_result[0][f2 - 1] = round(test_result[1], 3)
            total_result[1][f2- 1] = round(test_result[2], 3)
            total_result[2][f2 - 1] = round(test_result[3], 3)
            total_result[3][f2 - 1] = round(test_result[4], 3)
